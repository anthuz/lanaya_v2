...
class CFormMyGuestbook extends CForm {
  	...
  	public function __construct($object) {
    	parent::__construct();
    	$this->objecyt = $object;
    	$this->AddElement(new CFormElementText('name', array('label'=>'Name:')));
    	$this->AddElement(new CFormElementTextarea('message', array('label'=>'Message:')));
        $this->AddElement(new CFormElementSubmit('add', array('callback'=>array($this, 'DoAdd'), 'callback-args'=>array($object), 'class'=>'btn btn btn-primary')));
  	}
  	...
  	public function DoAdd($form, $object) {
    	return $object->Add(strip_tags($form['name']['value']), strip_tags($form['message']['value']));
  	}
}