...
class CMModules extends CObject {
  	...
  	/**
  	  ...
  	  */
  	 public function ReadAndAnalyse($folder=null) {
  	 	$src = LANAYA_INSTALL_PATH.'/'.$folder;
  	 	echo $src;
  	 	...
  	 	while (($module = $dir->read()) !== false) {
  	 		if(is_dir("$src/$module")) {
  	 			if(class_exists($module)) {
  	 				...
  	 				$modules[$module]['isHelper']       = preg_match('/^CForm[A-Z]/', $rc->name);
  	 				$modules[$module]['hasSQL']        = $rc->implementsInterface('IHasSQL');
  	 				$modules[$module]['isManageable']  = $rc->implementsInterface('IModule');
  	 				$modules[$module]['isLanayaCore']   = in_array($rc->name, array('CLydia', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
  	 				$modules[$module]['isLanayaCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
  	 			}
  	 		}
  	 	}
  	 	$dir->close();
  	 ...
  	 public function Install() {
  	 	$allModules  = $this->ReadAndAnalyse('system');
	    $allModules  = array_merge($this->ReadAndAnalyse('site/controllers'),$allModules);
	    $allModules  = array_merge($this->ReadAndAnalyse('site/helpers'),$allModules);
	    $allModules  = array_merge($this->ReadAndAnalyse('site/models'),$allModules);
