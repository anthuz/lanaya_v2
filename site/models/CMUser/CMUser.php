...
class CMUser extends CObject implements IHasSQL, IModule, ArrayAccess {
	...
	/**
  	 * Constructor
  	 */
	public function __construct($lanaya=null) {
		parent::__construct($lanaya);
  	...
  	public function Init() {
	  	try {
     		...
     		$idRootUser = $this->db->LastInsertId();
      		...
      		$this->AddMessage('info', 'Successfully created the database tables and created a default admin user as root:root and an ordinary user as doe:doe.');
  	...
  	public function CreatePassword($plain, $algorithm=null) {
    	$password = array(
      		'algorithm'=>($algorithm ? $algorithm : CLanaya::Instance()->config['hashing_algorithm']),
      		'salt'=>null
    	);
